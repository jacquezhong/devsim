...
- `tools/create_equation.py`（封装好的公式插入工具）
...
直接导入 `tools/create_equation.py` 中的函数：
...
add_formula(para, r'Q_{rr} = \tau \cdot J_F', display_mode=True)

# 方法2：快速添加（自动创建新段落）
add_simple_omml(doc, r'\alpha + \beta = \sqrt{\gamma^2 + \delta^2}')
```

**关键要点**：
1. **LaTeX 清理**：`clean_latex()` 自动处理多字母下标（如 `Q_rr` → `Q_{rr}`）
2. **XSLT 路径**：工具自动定位 `tools/MML2OMML.XSL` 文件
3. **命名空间**：自动添加 `xmlns:m` 命名空间，确保 Word 正确识别
4. **显示模式**：`display_mode=True` 时公式居中显示
