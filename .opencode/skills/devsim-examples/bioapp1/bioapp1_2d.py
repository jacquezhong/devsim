...
能力标识: bioapp_2d_ion_channel

工作思路:
1. 导入Gmsh生成的2D圆盘网格（disk_2d.msh）
...
- 圆柱坐标系（轴对称）
- 多区域多界面
- 需要Gmsh网格文件 disk_2d.msh

依赖:
- disk_2d.msh 网格文件（需预生成）
...
def run_bioapp1_2d_simulation(
    voltage=1.0,                    # 外加电压 (V)
    dna_charge_density=2e21,        # DNA电荷密度 (/cm^3)
    zero_charge_first=True,         # 先求解零电荷参考
    output_prefix="bioapp1_2d",
    # 网格参数
    mesh_file="disk_2d.msh",
    device_name="disk",
    # 区域定义
    regions_config=None,
    # 求解器参数
    ...
    注意:
        运行前需要确保 disk_2d.msh 文件存在
        可使用 Gmsh 生成: gmsh disk_2d.geo -2
    """
    ...
    try:
        ...
        result["converged"] = False
        result["error"] = str(e)
        result["hint"] = "Ensure disk_2d.msh exists (generate with: gmsh disk_2d.geo -2)"
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        voltage = 1.0
        print(f"Usage: python bioapp1_2d.py <voltage>")
        print(f"Using default voltage: {voltage}V")
    else:
        voltage = float(sys.argv[1])
    
    result = run_bioapp1_2d_simulation(voltage=voltage)
    
    if result.get("converged"):
        print(f"\n仿真成功: {result['output_file']}")
    else:
